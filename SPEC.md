...
**The sharesOf fallback is safe for a non-rebasing token and wrong-unit for a broken rebasing one.** `_shares()` reads `sharesOf`, requires exactly 32 bytes, and falls back to `balanceOf` otherwise. The length check is necessary: the NVDAx proxy returns padded data for selectors it does not implement rather than reverting, and a caller that trusts the return decodes nonsense as a share count. Verified by a token that returns 68 bytes; the check rejects it.

But the fallback itself carries an assumption. For a token where balance and shares are the same quantity, comparing `balanceOf` against a share-denominated target is correct. For a *rebasing* token whose `sharesOf` is broken or absent, the comparison is wrong by exactly the multiplier, and the contract does not revert — it silently compares the wrong unit. Every xStock tested implements `sharesOf` correctly, so this is not reachable through the intended basket, but baskets are owner-composed and nothing prevents an owner adding a token where it bites. Recorded as a limitation rather than fixed, because reverting instead would exclude every non-rebasing token from a basket.
