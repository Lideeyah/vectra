...
**What the re-measurement of the thin tail can and cannot establish.** NKEx, IRENx and RKLBx are graded because the spread from 0.0002% to 2.92% was the contrast the depth story rested on. But they were selected as "the three worst" *by the flawed method*, so that ranking is itself an artifact of it, and the genuinely thinnest assets may be elsewhere. If these three collapse toward the rest, the honest conclusion is not that the tail was found and then disproved; it is that the old method could not identify a tail at all. Establishing whether a real tail exists would require a ladder across the whole quotable set.
